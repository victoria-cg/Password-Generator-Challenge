...
//Assignment code provided by EdX for this homework challenge is at lines 4, 7, and 65-75 
...
if (length <8 || length >128 || (isNaN (length))) {
  ...
  return ""; //returns an empty string from the invalid input rather than letting the page continue to run more confirm booleans
}
...
//Ask if user wants lowercase alphabet characters
...
if (!includeUpperAlpha && !includeLowerAlpha && !includeNumbers && !includeSpecialChar) {
  ...
  return "Password selections invalid: please select character types to include. Click 'Generate Password' to try again."; //returns string with error message if no character types selected.
}
...
if (includeUpperAlpha) { 
  includedChar = includedChar.concat(upperAlpha); //this will combine the empty string in the new variable 'includedChar' with the string in the variable 'upperalpha' and make that the new value of 'includedChar' 
...
if (includeLowerAlpha) {
  includedChar = includedChar.concat(lowerAlpha); //continue same concatenation process in this section to add all character types selected by user input booleans
...
// Write password to the #password input. The code below on lines 65-75 is the starter code provided by the bootcamp
